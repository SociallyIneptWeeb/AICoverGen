...
from backend.generate_song_cover import run_pipeline as _run_pipeline
...
def complete_audio_ext(incomplete: str) -> list[str]:
    """
    Return a list of audio extensions that start with the incomplete string.
    ...
    -------
    list[str]
        The list of audio extensions that start with the incomplete string.
...
@song_cover_app.command(no_args_is_help=True)
def run_pipeline(
    source: Annotated[
        str,
        typer.Argument(
